...
import { CRASH_ISSUE_TOOL_NAME, crashIssueTool } from "@nakama/core/tools/crash-issue";
...
describe("omitUnavailableBuiltinTools", () => {
  ...
  test("drops crash_issue unless a repository is configured", () => {
    ...
    expect(omitUnavailableBuiltinTools(tools, true, false).map((tool) => tool.name)).toEqual([
      "web_search",
    ]);
    expect(omitUnavailableBuiltinTools(tools, true, true).map((tool) => tool.name)).toEqual([
      "web_search",
