...
describe("formatToolActivityLabel", () => {
  test("formats common read/search/fetch tools", () => {
    expect(formatToolActivityLabel("read_file", { path: "SOUL.md" })).toBe(
      "Reading SOUL.md"
    );
    ...
    expect(
      formatToolActivityLabel("knowledge_base_search", { query: "pricing" })
    ).toBe("Searching docs · pricing");
    expect(
      formatToolActivityLabel("web_fetch", { url: "https://example.com/docs" })
  ...
  test("falls back to tool name", () => {
    expect(formatToolActivityLabel("custom_tool", {})).toBe(
      "Using custom_tool"
    );
  });
});
