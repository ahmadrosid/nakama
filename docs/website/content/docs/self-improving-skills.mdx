...
Org admins: **Control center → Workspace → Organization → Skill write approval → Gate settings**.
...
Platform admins managing a specific bot can also open **Agent**, select the profile, and use the **Proposals** tab.
...
Org admins: **Control center → Workspace → Organization**, on the **Learn after a turn** card (next to Skill write approval). Platform admins can also flip it on after opening **Agent** and selecting the profile.

- Default is **off** (no extra review pass).
- Optionally override per profile: flip that profile's switch. **Use org default** follows the org again.

![Learn after a turn switch on an agent settings page](/screenshots/learn-after-a-turn-profile.png)
...
| **Where admins manage it** | **Control center → Workspace → Organization → Skill write approval** | **Control center → Workspace → Settings → Org Memory** |
...
Org admins: **Control center → Workspace → Organization → Skill curator**.
