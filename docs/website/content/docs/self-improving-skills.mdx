...
| **Discord, Telegram, WhatsApp, Slack** | No | Yes (when enabled) |
...
Every profile receives the default bundled skills after sync, including `manage-skills`, so skill-authoring capability is available without assigning that skill manually. Super Bot also receives its Super Bot-specific skills.
...
In **web** or **CLI** chat (not Telegram, WhatsApp, Discord, Slack, or automations), type:
...
When enabled, after a **successful** turn in an interactive chat (web, CLI, Discord, Telegram, WhatsApp, or Slack) that looks complex (many tool calls, or a tool error recovery) — and only if the agent did **not** already save a skill that turn — Nakama runs a lightweight background review. Chat is not blocked while that runs. It uses an extra model call on eligible turns, so leave it off unless you want the safety net.
...
These numbers update during **web** and **CLI** chat. They do not change from Telegram, WhatsApp, Discord, Slack, or automations — those channels do not run the skill-authoring flow today.
