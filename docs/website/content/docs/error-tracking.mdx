...
- The **platform admin** role in Nakama. Organization admins cannot configure
  error tracking.
...
**2. Paste it into Nakama.** As a platform admin, open **Integrations**, then
**Error tracking**, paste the DSN, and press **Save**.
...
**3. Press Send test event.** A platform admin can send the test. The badge
turns to **Sending** once a DSN is saved, and the button tells you whether the
...
| Save or clear the DSN | Platform admin only |
| Send a test event | Platform admin only |

The DSN applies to the whole installation, not to one organisation. Only a
platform admin can change or test it. There is no per-org error tracking,
because a crashed process does not belong to an organisation.
