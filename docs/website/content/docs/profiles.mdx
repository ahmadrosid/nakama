...
A packed skill's `tool.py` and declared `scripts/*.py` run as subprocesses on
the server host, so they are written to the imported workspace only for a
platform-admin import. An org-admin import lists them in the preview as skipped
and installs the skill's instructions without its Python.
