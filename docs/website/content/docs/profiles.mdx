...
Plugin assignments travel as plugin id, version, and contribution references. Import connects them only when the destination organization already has that plugin approved and compatible. A profile pack never copies plugin code into ordinary custom-tool or skill folders. See [Plugins](/plugins).
