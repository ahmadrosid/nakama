...
2. Creates a **Super Bot** profile for that org (`isSuper: true`)
3. Assigns the standard builtin tool set and default bundled skills to both profiles
4. Initializes each profile's soul directory on disk

The default profile is named **Default Bot** and gets a unique id (not a fixed
string like `default`). Super Bot is also created with a unique id.
...
| Super bot | Automatically created once per org with `isSuper: true`; it has the default tools and bundled skills plus Super Bot-specific capabilities |
...
Platform admins create extra profiles inside the **active org**. Each new profile gets its own soul directory and starts with the standard builtin tools and default bundled skills when those capabilities are installed on the server. On startup, Nakama backfills the default bundled skills onto every profile, including profiles created before the current bundled set. Assign more tools, MCP servers, and skills from the dashboard.
...
### User context and app-user sessions

Setup answers are personal context for the authenticated user who completed the
wizard. `USER.md` is stored per user within the organization, so two members can
have different roles, preferences, and working context. It is not shared
organization-wide.

An API-key session with an `appUserId` is a separate app-user session. It does
not load the authenticated organization user's `USER.md`; its private workspace
and artifacts are scoped to that stable app-user id. Keep each app user's
sessions and private files separate from every other app user.
...
Memory writes and archives use bundled skills (`update-profile-memory`, `archive-profile-memory`) with the profile's file tools — not separate memory builtins. Artifact saves use `save-artifact` with `write_file`. Every profile receives the default bundled skills, including these skills, when they are installed on the server; Super Bot also receives its Super Bot-specific skills.
