...
| `artifacts/` | No (**Browse Files → Artifacts**, download API, web chat preview) | `save-artifact` skill + `write_file` or `write_docx` |
| Knowledge base | Via `knowledge_base_search` when assigned | Upload from **Browse Files → Knowledge**; search at runtime |
...
Each profile also sees the default Nakama documentation source in **Browse Files → Knowledge**:
...
Tuning an agent and then wanting a close variant is common, so open **Agent**, select the profile, and choose **Clone agent**. It creates `{name} (copy)` in the same organization and selects it.
...
1. Open **Agent**, select a profile, and choose **Export** to download a `.zip`
2. On **Agent**, choose **Import**, preview what will be created or skipped, then confirm
