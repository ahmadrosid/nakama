...
2. **A model, set in AI Providers.** Pick **Image generation model** in the same card group as the vision and transcription models. The only allowlisted choice today is `gpt-image-2`.

Until a model is set, the tool refuses to generate images. The card shows **Not configured** in that state, and **No OpenAI provider** when the provider is the missing half.
