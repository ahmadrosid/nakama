...
- User context from `USER.md`, when available. Setup answers and `USER.md` are stored per authenticated user within the active organization, not shared organization-wide.
- The user's timezone
...
- Slack behavior when the message comes from a Slack DM or channel thread, including the channel audience and Slack Markdown rules

An API-key session with an `appUserId` is separate from the authenticated
organization user's session. It does not load that user's `USER.md`; its private
workspace and artifacts are scoped to the stable app-user id, so keep each app
user's session and files separate.
