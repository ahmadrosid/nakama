...
- **Audio transcription** (Telegram voice notes) — needs a transcription model in **Settings → Audio transcription model**, backed by an OpenAI provider or an OpenAI-compatible provider with a base URL (ChatGPT Plus/Pro cannot replace this)
...
1. In **Settings → LLM providers → Add provider**, choose **Custom (OpenAI-compatible)**.
2. Set **Base URL** to your local endpoint (for example `http://localhost:8000/v1` or `http://100.64.0.1:8000/v1` for a Tailscale peer).
3. Enter an API key. Local backends often ignore it, but the field is required — use any non-empty placeholder (`local-key`, `ollama`, etc.) if your server does not verify one.
4. Add at least one model under **Models** (`gpt-image-2`, a Whisper id, etc.). For a model you set as the transcription or image-generation model, enable the matching capability (image **Vision** toggle for image parsing) under **Settings → LLM providers**.

### Image generation

Set **Image generation model** in Settings to select `gpt-image-2` against the compatible instance:
...
```text
p-my-local::whisper-1
```

where `p-my-local` is the id of the local provider instance. Nakama sends a multipart `POST {baseUrl}/audio/transcriptions` with the audio `file` and the `model` name — the same contract as OpenAI's transcriptions endpoint.
...
curl -sS "$BASE_URL/v1/images/generations" \
  ...
  -d '{"model":"gpt-image-2","prompt":"a cat","n":1,"output_format":"png","size":"1024x1024"}'
```
