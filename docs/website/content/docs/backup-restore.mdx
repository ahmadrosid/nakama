...
Platform admins can export local Nakama data from **Control center → Workspace → Settings → Export ZIP**.
...
Restored plugins stay **disabled** until an organization admin enables them in **Control center → Agent tools → Plugins**. If a restored package no longer matches its stored hash, that release stays unavailable and must be installed again.

## Profile packs (single agent)

To move **one** profile without a full-root backup, open **Agent**, select the profile, and choose **Export** or **Import**. See [Export and import an agent pack](/profiles#export-and-import-a-profile-pack).
