...
2. Select the organization, then open **Agent → your agent → Channels → WhatsApp**
...
1. In **Agent → your agent → Channels → WhatsApp**, generate or copy the pairing code
...
2. Wait for the QR code to appear in **Agent → your agent → Channels → WhatsApp**
...
- group chats reply to a mention of the linked number, a reply to a Nakama message, or a `/command`. Turn off **Require @mention in groups** in **Agent → your agent → Channels → WhatsApp** if anyone in the group should talk without tagging the bot
...
2. the WhatsApp bridge is running in the selected agent's **Agent → your agent → Channels → WhatsApp** settings
3. the WhatsApp account is linked
4. the linked number shown in **Agent → your agent → Channels → WhatsApp** is the one you are messaging from

### Pairing code does not work

Check these first:

1. generate a fresh pairing code from **Agent → your agent → Channels → WhatsApp**
...
1. use **Reconnect with QR** in **Agent → your agent → Channels → WhatsApp**
...
Turn off **Require @mention in groups** in **Agent → your agent → Channels → WhatsApp** to answer every group message without tagging the bot.

The sender must be the linked WhatsApp number, or a number listed under **Agent → your agent → Channels → WhatsApp → Allowed numbers**. Add country code, like `+62812…`. Reply to another person's message while mentioning Nakama if the agent should see that text.
...
Nakama preserves a linked account when its existing settings identify one owner. Ambiguous connections stay stopped. Open the target agent's **Agent → your agent → Channels → WhatsApp** settings to configure the connection. Installation-wide connections require a platform admin. Stop manually launched workers before reconnecting.
