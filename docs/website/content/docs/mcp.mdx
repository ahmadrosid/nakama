...
1. Add the server with its URL and no headers, then save. **Test connection** reports a
   `401` for these, which is expected: the sign-in has not happened yet
2. Nakama opens the provider's authorization page in a new tab. If a popup blocker eats it,
   use the link in the banner on the MCP page
...
4. The provider sends you back to Nakama, which finishes the connection and caches the
   tools. The MCP page flips to **connected** on its own
...
- **"This server requires authorization"** - the OAuth sign-in never finished. Click **Connect** to get a fresh link, and check the public URL matches the origin you open the dashboard on
