...
1. Open **Control center → Agent tools → MCP**, then select **Add server**
...
1. Open **Agent** and select a profile
2. In the **MCP** tab, click **Add MCP server**, then pick an existing server or create a new one
...
- **A reachable public URL.** The provider redirects your browser back to
  ...
  **Control center → Workspace → Settings → Public web URL** (or `NAKAMA_WEB_PUBLIC_URL`) for anything other than a local install
