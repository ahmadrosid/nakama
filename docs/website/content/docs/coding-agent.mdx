...
**Readiness checks:** Opening Integrations or loading harness settings runs a light check (binary on `PATH` + provider passthrough active). Use **Run readiness check** to run a deeper exec probe that spawns the harness with passthrough env. `configured` and `activeHarnessId` reflect only the **selected** harness — Nakama does not fall back to another ready harness.
...
| Claude Code asks for `/login` | Provider passthrough off or spawn env not applied — restart `dev:server`; launch output should say passthrough is active; check Settings → Provider |
| Provider passthrough inactive / no provider | Configure a compatible model provider in Nakama Settings for the launch profile |
