...
Artifacts are files your agent creates and saves for you — reports, summaries, generated text, Word documents, and other outputs you can preview or download later from **Browse Files → Artifacts** or from web chat.
...
Search uploaded knowledge base documents for relevant facts. **Browse Files → Knowledge** can also show inherited URL sources, such as the Nakama documentation index at `llms.txt`; use `web_fetch` on that index and on specific `.md` pages — not `knowledge_base_search`.
...
**Custom search back-end:** Set a provider under **Control center → Agent tools → AI Providers → Web search** (Exa, Firecrawl, or any endpoint that answers with JSON search results) to run the search on Nakama instead of the model provider. This removes the provider restriction above and returns `{ provider, query, results: [{ title, url, snippet }] }`. Leave it on **Built-in** to keep the provider-hosted search.
...
List, read, search, and send email through the deployment mailbox configured in **Control center → Agent tools → Tools**.
...
Org admins configure these from **Control center → Agent tools → Tools**.

### Web search

Requires an OpenAI or Anthropic provider with a configured API key, or an Exa or Firecrawl search back-end.

Org admins can point `web_search` at an external search API from **Control center → Agent tools → AI Providers → Web search**: pick **Exa** or **Firecrawl**, then supply the API key. The key is stored in the `[web_search]` section of `~/.nakama/config.ini` and is only ever returned masked.
...
Platform admins can export and import the whole local Nakama data root from **Control center → Workspace → Settings**.
...
Custom tools live in `~/.nakama/tools/` (follows `NAKAMA_CONFIG_DIR` if set) and are registered by a platform admin from **Control center → Agent tools → Tools**. Each tool points at a module file and can then be assigned to agents like any builtin.
