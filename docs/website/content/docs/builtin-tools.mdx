...
**Availability:** Off by default. `bash` is not assigned to the org Super Bot and cannot be assigned to any profile until the operator sets `NAKAMA_TENANT_BASH=1`. Org admins can reach the Super Bot in chat, so leaving the tool on by default hands every tenant a shell that runs as the Nakama server process. Set `NAKAMA_TENANT_BASH=1` together with `NAKAMA_BASH_BACKEND=microsandbox` when tenants need a shell; `host` execution for tenants is a deliberate, instance-wide decision.
...
**Tenant policy:** `NAKAMA_TENANT_BASH` (`0` default, `1` enables) decides whether the tool may be attached to a profile at all. While it is off, the Super Bot is seeded without `bash`, an existing grant is revoked on the next server start, and tool assignment requests for `tool_bash` are rejected with `403` — including the Super Bot's own `assign_tool_to_profile`. The tool definition still exists, so re-enabling needs no migration.
