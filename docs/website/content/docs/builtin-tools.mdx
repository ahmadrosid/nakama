...
**Coding-agent spawn env:** When `codingAgent: true` and the command starts with a known harness binary (`codex`, `claude`, `opencode`, or `pi`), Nakama merges provider passthrough env vars on the server before spawn. See [Coding agent — Provider passthrough](/coding-agent#provider-passthrough). Optional `env` keys are merged on top (credential keys cannot override passthrough).
