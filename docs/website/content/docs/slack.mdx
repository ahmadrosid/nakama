...
1. Open **Agent → profile → Channels → Slack** for the profile that should answer in Slack (you need to be an org admin)
...
On **Agent → profile → Channels → Slack**:
...
Use the worker controls on **Agent → profile → Channels → Slack** to start the worker. The profile's Channels list shows Slack as **Connected** once the bridge is running and at least one member can chat.
...
Pairing members one by one does not scale to a whole team. Turn on **Everyone in the workspace** on **Agent → profile → Channels → Slack** and click **Save**: every full member of your Slack workspace can then use the app with no pairing.
...
2. On **Agent → profile → Channels → Slack**, paste the ID into **Allowed users** and press **Enter**. It turns into a badge; paste several IDs at once and each becomes its own badge. Click **×** on a badge to remove it.
3. Click **Save**
...
1. The worker is running on **Agent → profile → Channels → Slack**
