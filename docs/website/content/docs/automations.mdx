...
The two do connect. An automation turn holds the workflow tools, so a prompt like
`Run my "Morning brief" workflow (id wf_123)` puts a saved recipe on a schedule
without any extra wiring. Press **Schedule** on a workflow to start that
conversation with the id already filled in.
