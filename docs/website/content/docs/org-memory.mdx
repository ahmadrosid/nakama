...
The three org memory tools are added when a profile has at least one assigned
or resolved tool. They are not attached to a profile with no tools, and you do
not assign these three from the dashboard like other builtins.
...
Viewers cannot call these tools. Automations and sub-agents run with member-level org context when the profile has tools available.
...
|------|------|--------|----------------|------------------|-------------|
| **admin** | yes | yes | yes | yes | yes, when the profile has tools |
| **member** | yes | yes | no | yes | yes, when the profile has tools |
