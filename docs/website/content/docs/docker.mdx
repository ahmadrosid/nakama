...
(v0.4.31) with no LLM provider and no channel workers:
...
- `NAKAMA_DATA_VOLUME` sets the Docker volume name. The default is
  `nakama-data`.
...
The reset script removes the container named `nakama`, every container that
