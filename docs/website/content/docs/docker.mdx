...
- Use `/healthz` for liveness and `/readyz` for readiness on port 4310. Readiness returns 200 when the migrated database can be read, or 503 when unavailable, including during database restore. Neither probe needs login or checks external providers or worker connectivity. The existing `/health` behavior is unchanged.
