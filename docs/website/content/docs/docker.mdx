...
Profiles do not get the `bash` tool unless you pass `NAKAMA_TENANT_BASH=1`; until then no Super Bot or tenant profile can reach a shell. If you do enable it inside Docker, `NAKAMA_BASH_BACKEND=microsandbox` (with `/dev/kvm`) is the only backend that keeps tenants off the shared container filesystem.
