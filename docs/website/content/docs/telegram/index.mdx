...
QR setup connects your Telegram bot without copying a token from BotFather. An organization admin can open **Agent → profile → Channels → Telegram**, then select **Create with QR**.
...
The manager's [source code is public](https://github.com/ahmadrosid/nakama/tree/main/packages/telegram-manager). Operators can inspect and host it themselves, or configure their own manager. Public code does not verify which version a hosted service runs. Use the manual BotFather setup below if you do not want to grant a shared manager access.
...
Open **Agent → profile → Channels → Telegram** in the Nakama web app, then:

1. Paste the bot token
2. Check that you opened the profile you want to connect
3. Save

![Paste the bot token from @BotFather on Agent → profile → Channels → Telegram](/screenshots/telegram-bot-token.png)
...
Existing connections move automatically when Nakama can identify their owner. If ownership is unclear, the worker stays stopped. Open the owning profile's **Agent → profile → Channels → Telegram** page and claim the existing connection. Only a platform admin can claim an installation-wide connection.
...
1. Open **Agent → Control center → LLM providers** in the Nakama web app
...
1. Copy the pairing code from **Agent → profile → Channels → Telegram**
...
1. Open **Agent → profile → Channels → Telegram**
