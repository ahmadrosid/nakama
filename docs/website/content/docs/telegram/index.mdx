...
An organization admin can open **Agent → your agent → Channels → Telegram**, and select **Create with QR**.
...
![Paste the bot token from @BotFather on Agent → your agent → Channels → Telegram](/screenshots/telegram-bot-token.png)
...
Existing connections move automatically when Nakama can identify their owner. If ownership is unclear, the worker stays stopped. Open the target agent's **Agent → your agent → Channels → Telegram** settings to configure the connection. Only a platform admin can resolve an installation-wide connection.
...
![Choose an OpenAI audio transcription model under AI Providers](/screenshots/telegram-audio-transcription.png)
