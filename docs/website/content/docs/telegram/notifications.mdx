...
- `X-API-Key` — the destination API key
- `Idempotency-Key` — a unique id per event (max 128 characters). Replays of the same key are rejected and do not send again. Claims are kept for 7 days (including after a failed Telegram send) so a lost success response cannot unlock a second delivery.
