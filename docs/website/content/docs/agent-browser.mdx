...
| **Agent settings** | Assign the skill. **Install** the CLI and Chrome on the server. **Add bash** when needed |
...
### 1. Assign the skill from the agent settings

Platform admins:

1. Open **Agent** and select the profile.
...
Then open the selected agent's **Skills** section. Assign the skill. Use **Add bash** if needed.
...
| `command not found` / `ENOENT` | Use **Install** in the selected agent's **Skills** picker, or install manually (`npm install -g agent-browser && agent-browser install`) |
