...
Answers and the resulting `USER.md` are saved for the authenticated user within this organization, then added to that user's conversations. They are not shared with every member of the organization. See [How agent prompts work](/agent-prompt) for where they land in the prompt.
