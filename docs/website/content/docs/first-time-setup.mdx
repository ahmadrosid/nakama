...
Add your first LLM provider and pick a default model. You can add more providers later in **Control center → Agent tools → AI Providers**.
...
Every field is optional and **Set up later** skips the whole step. Anything you had typed is kept as a draft in this browser and shows up again in **Control center → Workspace → Settings**. Press Enter in any field to move to the next screen. You can fill this in or change it at any time from **Control center → Workspace → Settings**, or from the account menu in the sidebar. That is also where to start if your install skipped the wizard through the [first-boot seed](/docker#first-boot-seed-optional).
...
If chat fails, reopen **Control center → Agent tools → AI Providers** and confirm the API key and per-agent model selection.
