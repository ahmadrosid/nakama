...
trimmed environment (`PATH`, `NAKAMA_CONFIG_DIR`, `NAKAMA_WORKSPACE_ROOT`) and
...
Org admins can enable **skill write approval** under **Control center → Workspace → Organization → Skill write approval**. When the gate is on, skill changes wait for admin review before they go live; see [Self-improving skills](/self-improving-skills). Unused agent- and human-authored skills can be archived (moved, never deleted) by the optional **skill curator** on the same Organization page. A separate opt-in can also **consolidate** overlapping agent skills with an LLM; see [Self-improving skills](/self-improving-skills).
