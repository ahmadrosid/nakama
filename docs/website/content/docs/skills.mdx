...
staged for an admin to approve; once it is on, the skill's code runs like any
other. With approval off, the skill still reads as instructions, and calling its
tool returns an error saying the code is not runnable here.
