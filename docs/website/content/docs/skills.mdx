...
trimmed environment (`PATH` and `NAKAMA_WORKSPACE_ROOT` only — the deployment
config dir and every provider key stay out of reach) and 30 seconds to answer.
...
### Python in a profile pack

Because a skill's Python runs on the server host, importing a profile pack only
installs `.py` files when a **platform admin** approves it. An org-admin import
lists every packed `tool.py` and declared `scripts/*.py` in the preview as
skipped and never writes them to the workspace, so the imported skill lands as
instructions only. See [Profiles → Export and import a profile pack](/profiles#export-and-import-a-profile-pack).
