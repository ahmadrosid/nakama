...
Every profile receives the default bundled skills above when they are installed and synced on the server, including `manage-skills` for skill authoring. Nakama backfills missing default bundled skills onto existing profiles during startup. Super Bot additionally receives its Super Bot-specific skills, while opt-in skills such as `agent-browser` still require assignment.
