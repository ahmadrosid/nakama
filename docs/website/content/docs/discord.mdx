...
| --- | --- | --- |
| **Bot token** | **Bot** | Yes — paste in **Agent → profile → Channels → Discord** |
...
You can also use the **Invite bot to server** link on **Agent → profile → Channels → Discord** after saving your token — Nakama generates an invite with the right permissions.
...
An organization admin can open **Agent → profile → Channels → Discord** and select the profile that should answer. The route is scoped to that profile, so the connection is fixed to it; there is no separate default-profile selector. Then:
...
1. Copy the pairing code from **Agent → profile → Channels → Discord** (click **Regenerate** if needed)
...
On **Agent → profile → Channels → Discord**, use the **Bridge worker** controls to start the worker. The worker is scoped to the organization and profile in the route.
...
1. Open **Agent → profile → Channels → Discord**
...
If an older connection cannot be assigned automatically, it stays stopped. Claim it from **Agent → profile → Channels → Discord**. Installation-wide connections require a platform admin.

## Rotating the bot token

After you replace the Discord bot token, restart this profile's Discord worker. The long-lived gateway process keeps the token it loaded at startup, so saving alone does not rotate the running connection.

## Running the worker directly

The dashboard supplies the scope automatically. A direct run must set both values so the worker reads the correct profile-scoped config:
...
Save the Discord connection in the same Nakama config directory first. Set `NAKAMA_SERVER_URL` and `NAKAMA_CONFIG_DIR` as needed for a separate server or non-default config root.
...
3. Save, then restart the Discord bridge worker in **Agent → profile → Channels → Discord**
...
2. The Discord bridge worker is running (start it from **Agent → profile → Channels → Discord**)
