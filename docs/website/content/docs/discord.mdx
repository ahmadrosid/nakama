...
Sign in as a platform admin and open **Agent → your agent → Channels → Discord**, then:
...
If an older connection cannot be assigned automatically, it stays stopped. Open the target agent's **Agent → your agent → Channels → Discord** settings to configure the connection. Installation-wide connections require a platform admin.
