...
TinyClaw is an **MCP client**. It connects to MCP servers you register, discovers the tools they expose, and bridges those tools into a profile's chat loop so the model can call them like any other tool.
...
MCP is how you plug those in without writing custom code inside TinyClaw. The [Model Context Protocol](https://modelcontextprotocol.io) is a standard for exposing tools to LLMs, and there is a growing ecosystem of ready-made MCP servers for things like databases, APIs, issue trackers, and knowledge sources.
...
TinyClaw supports two ways to reach an MCP server:
...
TinyClaw opens a streamable HTTP connection to the URL and reuses it across turns. Put any auth tokens or custom headers in the `headers` map.
...
TinyClaw spawns the command as a child process and talks to it over stdin/stdout using the MCP stdio transport. `args` and `env` are optional.
...
For stdio servers, TinyClaw runs one process **per profile** and sets the working directory to that profile's soul directory (`~/.tinyclaw/profiles/{profileId}/`). This keeps each profile's stdio server isolated. HTTP servers share a single connection regardless of how many profiles use them.

## How tools appear to the model

When a chat runs, TinyClaw looks at the profile's assigned MCP servers, reads each server's **cached tools**, and exposes them to the model as regular tools.
...
The model sees these tools alongside the profile's builtin tools and can call them in the same turn. When the model calls one, TinyClaw forwards the call to the connected MCP server and returns the result.

## The connect and sync lifecycle

MCP servers are not queried live on every chat turn. Instead, TinyClaw **caches the tool list** when it connects. This is why there is a connect/sync lifecycle.

| State | What it means |
|-------|---------------|
| `connected` | TinyClaw has an active client connection and a cached tool list |
...
Syncing refreshes the cached tool list from an already-connected server. Run this when the server has added, removed, or renamed tools and you want TinyClaw to pick up the change.

### Restart behavior

When TinyClaw starts up, it automatically reconnects every **enabled** MCP server. Disabled servers are left disconnected. If a server cannot be reached at startup, TinyClaw logs a warning and continues — the server's status becomes `error` and can be reconnected from the dashboard.
...
Use **Import JSON** in the add dialog, or simply paste the JSON onto the form. TinyClaw parses the standard shape:
...
HTTP headers and stdio environment values are often secrets. TinyClaw treats them carefully:
...
- [Builtin tools](/builtin-tools) — the tools that ship with TinyClaw
